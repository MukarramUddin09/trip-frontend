    ...
    <div className="space-y-6">
      ...
      <GlassCard className="relative overflow-hidden p-6 md:p-8">
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-2 lg:grid-cols-[1fr_1fr_auto]">
          ...
          <IndigoButton className="lg:self-end" onClick={quickSearch}>
            Find Best Options →
          </IndigoButton>
        </div>
      </GlassCard>
            ...
            <tbody>
              {loading
                ? Array.from({ length: 3 }).map((_, i) => (
                    <tr key={i}>
                      <td colSpan={4} className="py-3"><Skeleton className="h-4 w-full" /></td>
                    </tr>
                  ))
                : trips.map((t) => (
                    <tr key={t._id} className="border-b border-border/60">
                      <td className="py-3 capitalize">{t.type === "hotel" ? <Hotel className="h-4 w-4 inline" /> : <Train className="h-4 w-4 inline" />} {t.type}</td>
